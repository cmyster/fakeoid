...
func TestAgent7_NewAgent7_ReturnsNonNil(t *testing.T) {
	dir := t.TempDir()
	...
	a := NewAgent7(dir, enrichedFile, changePlanFile, "test output here", handoffFile)
	require.NotNil(t, a)
	assert.Equal(t, 7, a.Number())
	assert.Equal(t, "Course Corrector", a.Name())
}

func TestAgent7_ImplementsAgent(t *testing.T) {
	dir := t.TempDir()
	enrichedFile := filepath.Join(dir, "001-test-enriched.md")
	changePlanFile := filepath.Join(dir, "001-test-change-plan.md")
	handoffFile := filepath.Join(dir, "001-test-handoff.md")
	...
	a := NewAgent7(dir, enrichedFile, changePlanFile, "test output", handoffFile)
	var _ Agent = a // compile-time interface check
}

func TestAgent7_HandleResponse_AlwaysComplete(t *testing.T) {
	a := &Agent7{}
	for i := 0; i < 5; i++ {
...
func TestAgent7_SystemPrompt_ContainsAllSections(t *testing.T) {
	dir := t.TempDir()
	enrichedFile := filepath.Join(dir, "001-test-enriched.md")
	changePlanFile := filepath.Join(dir, "001-test-change-plan.md")
	handoffFile := filepath.Join(dir, "001-test-handoff.md")
	...
	a := NewAgent7(dir, enrichedFile, changePlanFile, "TEST_OUTPUT_CONTENT", handoffFile)
...
func TestAgent7SystemPrompt_ContainsFourSections(t *testing.T) {
	prompt := Agent7SystemPrompt("enriched", "changeplan", "testout", "handoff")
